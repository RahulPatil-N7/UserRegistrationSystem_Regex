...
public class UserRegistrationLamda {
    public static void main(String[] args) {
        ...
        System.out.println(firstName.validate("[A-Z]{1}[a-zA-Z]{2,}", "Rahul"));
        
        //LastName validation
        LambdaOperation lastName = (regex, input) -> {
            return "Last name validation is : " + Pattern.matches(regex, input);
        };
        System.out.println(lastName.validate("[A-Z]{1}[a-zA-Z]{2,}", "Patil"));
        ...
        System.out.println(mobileNumber.validate("[0-9]{2}[\\s][6789]{1}[0-9]{9}", "91 9967116186"));
        
        //Password validation
        LambdaOperation password = (regex, input) -> {
            return "Password  validation is : " + Pattern.matches(regex, input);
        };
        System.out.println(password.validate("(?=.*[A-Z]){1,}(?=.*[a-z]){1,}(?=.*[0-9]){1,}(?=.*[@#$%^&-+=()]){1}[a-zA-Z0-9@#$%^&-+=()]{8,}", "Rahul@123"));
    }
